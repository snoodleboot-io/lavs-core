...
class ProductResponseMapper:
    ...
    Centralises the column ordering (``id, name, description, created_at``)
    shared by the create, fetch-by-id and list queries so the ``created_at``
    timestamp is rendered to an ISO-8601 string in exactly one place.
    """
    ...
    def to_model(row: tuple[object, ...]) -> ProductResponseModel:
        ...
        Args:
            row: A row of ``(id, name, description, created_at)`` as returned by
                a ``SELECT`` against the ``products`` table. ``created_at`` is a
                :class:`datetime.datetime`; ``description`` may be ``None``.

        Returns:
            The populated :class:`ProductResponseModel`.
        """
        product_id, name, description, created_at = row
        rendered_created_at = (
